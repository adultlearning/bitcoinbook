Question: why can't someone just do DROP DROP TRUE as an unlocking script?
LOL answer: cuz unlock script comes first in stack
...
A new way of thinking: instead of paying to address, you pose a challenge. Whoever can solve the challenge can spend the UTXO. P2SH is a challenge that says: I want the solution SCRIPT to be exactly something. Thus, the solution scripts hash must have the same hash as my desired solution script 
...
<!--
1. after 90 days, if the transaction still hasn't been spent, then he can. but if any of the other paths are redeemed, then this particular transaction is no longer an UTXO and he has to wait another 90 days for any subsequent transactions.
2. 1, 2, 3
3. no. the first path is still valid. and he could have a signature for this transaction but not his private key for some reason
4. unlocking via path 1 and making a new transaction with this lock script.
5. there are other conditions to be met following the verify ones, but the checksig with no verify is the last statement so the last item on the stack will be true and the transaction unlocked.
-->
